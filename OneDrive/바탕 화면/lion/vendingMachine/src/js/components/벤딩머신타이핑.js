class vendingmachine {
  constructor() {
    ...
    this.btnReturn = vMachine.querySelector(".btn-get");
  ...
  bindEvents() {
    /**
     * 1. 입금 버튼 기능
     * 입금액을 입력하고 입금 버튼을 누르면 소지금 == 소지금 - 입금액, 잔액 == 기존 잔액 + 입금액이 돕니다.
     * 입급액이 소지금 보다 많다면 실행을 중단하고 "소지금이 부족합니다." 라고 쓰인 경고창을 띄웁니다.
     * 입금액 인풋창은 초기화됩니다.
     * */
    ...
    this.btnPut.addEventListener("click", (event) => {
      ...
      if (inputCost) {
        ...
        this.inputCostEl.value = null; // null을 ""로 바꿔도 똑같음
      }
    });
    ...
    this.btnReturn.addEventListener("click", (event) => {
      ...
      if (balanceVal) {
        this.myMoney.textContent =
          new.Itnl.NumberFormat().format(balanceVal + myMoneyVal) + " 원";
      ...
      btnsCola.forEach((item) => {
        item.addEventListener("click", (event) => {
          ...
          const balanceVal = parseInt(
            ...
            // forEach 문을 사용할 경우 반복의 종료가 불가능하다(return, break 작동 안함). 모드 원소를 순환할 필요가 없다면 비효율적.
            for (const iteme of stagedListItem) {
              // 클릭한 음료수가 내가 이미 선택한 아이템인지 탐색
              if (iteme.dataset.item === targetEl.dataset.item) {
                // 내가 클릭한 상품과 내가 담은 상품이 같을 경우
                iteme.querySelector(".num-counter").textContent++;
                isStaged = true;
                break;
              }
            }
